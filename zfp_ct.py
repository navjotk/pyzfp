...
class ZfpStream (ctypes.Structure):
    _fields_ = [("stream", ctypes.POINTER(BitStream)),
                ("minbits", ctypes.c_int), ("maxbits", ctypes.c_int),
                ("maxprec", ctypes.c_double), ("minexp", ctypes.c_double),
                ("exec", ZfpExecution)]


class ZfpField(ctypes.Structure):
    _fields_ = [("type", ctypes.c_int), ("nx", ctypes.c_int),
                ("ny", ctypes.c_int), ("nz", ctypes.c_int),
                ("sx", ctypes.c_int), ("sy", ctypes.c_int),
                ("sz", ctypes.c_int), ("data", ctypes.c_void_p)]


path = os.path.dirname(os.path.realpath(__file__))
libzfp = cdll.LoadLibrary("%s/zfp-0.5.3/lib/libzfp.so" % path)
libzfp.zfp_stream_set_accuracy.argtypes = (ctypes.POINTER(ZfpStream),
                                           ctypes.c_double)
libzfp.zfp_stream_set_precision.argtypes = (ctypes.POINTER(ZfpStream),
                                            ctypes.c_int)
libzfp.zfp_stream_set_rate.argtypes = (ctypes.POINTER(ZfpStream),
...
def compress(indata, tolerance=None, precision=None, rate=None, parallel=True):
    ...
    zfp_fields = {1: libzfp.zfp_field_1d, 2: libzfp.zfp_field_2d,
                  3: libzfp.zfp_field_3d}
    ...
    if tolerance is not None:
        ...
        libzfp.zfp_stream_set_rate(stream, rate, data_type, len(shape), 0)
    # Try multithreaded
    if(parallel):
...
def decompress(compressed, shape, dtype, tolerance=None, precision=None,
    ...
    libzfp.zfp_stream_set_bit_stream(stream, bitstream)
    libzfp.zfp_stream_rewind(stream)
    libzfp.zfp_decompress(stream, field)
